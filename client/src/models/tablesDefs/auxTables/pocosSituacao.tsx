...
import { queries, mutations } from './pocosSituacao.graphql';

const queryOne = {
  query: () => queries.queryOne,
  getObject: (data) => {
    const { pocos_situacao } = data;
    if (!pocos_situacao) return null;

    if (!(pocos_situacao.length > 0)) return null;

    const situacao = pocos_situacao[0];

...
import { queries, mutations } from './capSuperfSituacao.graphql';

const queryOne = {
  query: () => queries.queryOne,
  getObject: (data) => {
    const { cap_superf_situacao } = data;
    if (!cap_superf_situacao) return null;

    if (!(cap_superf_situacao.length > 0)) return null;

    const situacao = cap_superf_situacao[0];

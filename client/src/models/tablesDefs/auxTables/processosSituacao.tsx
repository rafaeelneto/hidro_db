...
import { queries, mutations } from './processosSituacao.graphql';

const queryOne = {
  query: () => queries.queryOne,
  getObject: (data) => {
    const { processos_situacao } = data;
    if (!processos_situacao) return null;

    if (!(processos_situacao.length > 0)) return null;

    const situacao = processos_situacao[0];

    return {
      main: enums.enum_processo_situ.get(situacao.situacao) || '',
...
const fields: AuxFieldType[] = [
  {
    ...
    getValue: (result) => {
      ...
      return {
        ...
        label: enums.enum_processo_situ.get(result.situacao),
      };
    },
    component(value, onChange) {
      return (
        <AutoCompleteEnum
          ...
          onChange={onChange}
          enumOptions={enums.enum_processo_situ}
        />
      );
    },
  },
